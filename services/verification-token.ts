...
export const selectVerificationTokenByToken = async (token: number, userId: number) => {
    return await db.select().from(verificationTokens)
        .where(sql`${verificationTokens.token} = ${token} and now() < ${verificationTokens.expiredAt} and ${verificationTokens.userId} = ${userId}`);
}